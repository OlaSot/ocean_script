...
export function ServiceCard({ title, description, index, image }: ServiceCardProps) {
  ...
  const hoverEffects = isServicesPage
    ? "bg-[url('/stagesBgWhite.webp')] bg-contain"
    : "hover:bg-[url('/Card.webp')] hover:shadow-lg cursor-pointer";


  const textColor = isServicesPage ? "text-black" : "text-white";
  const borderColor = isServicesPage ? "border-black" : "border-white";
  const bgColor = isServicesPage ? "bg-white" : "bg-transparent";
    ...
    >
      ...
      <div>
        <div className="flex items-center gap-2 mb-2">
          <span className={`w-3 h-3 rounded-full ${isServicesPage ? "bg-black" : "bg-white"}`}></span>
